...
function bucketByMonth(rows, months) {
  return months.map(m => rows.filter(r => {
...
function AdminAnalytics() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const rptBuckets = bucketByMonth(recentReports,  months);
      const reqBuckets = bucketByMonth(recentRequests, months);
      setMonthly(months.map((m, i) => ({
        month:    m.label,
        reports:  rptBuckets[i],
        requests: reqBuckets[i],
      })));
      ...
      reports.forEach(r => {
        ...
        brgyMap[b] = brgyMap[b] || { barangay: b, reports: 0, requests: 0 };
        brgyMap[b].reports++;
      });
      requests.forEach(r => {
        const b = r.barangay || 'Unknown';
      ...
      const sorted = Object.values(brgyMap)
        .sort((a, b) => (b.reports + b.requests) - (a.reports + a.requests))
        .slice(0, 8);
  ...
  const STAT_CARDS = [
    { label: 'Total Reports',   value: stats.reports,  accent: '#1d4ed8', iconBg: '#dbeafe', sub: 'Across all barangays',
    ...
    { label: 'Registered Users', value: stats.users,   accent: '#0369a1', iconBg: '#e0f2fe', sub: 'Total residents',
  ...
  const hasBarangay = barangayData.length > 0;
      ...
      <div className="off-main">
        ...
        <div className="off-content">
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <p className="off-page-sub" style={{ margin: 0 }}>System-wide reports and requests insights across all barangays</p>
          </div>
          ...
          {/* Row 1: monthly + status + top reporters */}
          <div className="off-analytics-grid" style={{ marginBottom: 20 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>System-wide over last 6 months</div>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                ...
                {fetching ? (
                  <div style={{ height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Spinner size={28} /></div>
                ) : statusDist.length === 0 ? (
              ...
              <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                ...
                <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 14 }}>Most active residents system-wide</div>

                {fetching ? (
                  <div style={{ height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Spinner size={24} /></div>
                ) : topReporters.length === 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px 0', gap: 8 }}>
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
          ...
          {/* Row 2: Barangay activity breakdown */}
          <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#1f2937' }}>Barangay Activity Breakdown</div>
                <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>Reports and requests per barangay (all time)</div>
              </div>
              <div style={{ display: 'flex', gap: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#6b7280' }}>
                  <span style={{ width: 10, height: 10, borderRadius: 3, background: '#93c5fd', display: 'inline-block' }} />Reports
            ...
            {fetching ? (
              ...
              <div style={{ height: 220, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#f8fafc', borderRadius: 12 }}>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="18" y="3" width="4" height="18"/><rect x="10" y="8" width="4" height="13"/><rect x="2" y="13" width="4" height="8"/></svg>
                <div style={{ fontSize: 13, color: '#9ca3af', fontWeight: 600 }}>No barangay data yet</div>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={barangayData} barGap={4} margin={{ left: 0, right: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="barangay" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis allowDecimals={false} tick={{ fontSize: 12 }} axisLine={false} tickLine={false} />
                  <BarTooltip contentStyle={{ borderRadius: 10, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontSize: 12 }} />
                  <Bar dataKey="reports"  name="Reports"  fill="#93c5fd" radius={[4,4,0,0]} />
                  <Bar dataKey="requests" name="Requests" fill="#7c3aed" radius={[4,4,0,0]} />
